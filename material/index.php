...
$whereConditions = [];
$whereConditions[] = "m.nama_material <> ''";

if ($cari_clean !== '') {
    $whereConditions[] = "(m.nama_material LIKE '%$cari_clean%')";
}

// FIX MUTLAK: Mengunci kategori menggunakan LOWER() dan TRIM() agar huruf besar/kecil tidak bikin bocor
if ($kategori_db !== '') {
    $kat_cari = strtolower($kategori_db);
    if ($kat_cari == 'stok') {
        $whereConditions[] = "(TRIM(LOWER(m.jenis_kategori)) = 'stok' OR TRIM(LOWER(m.jenis_kategori)) = 'stock' OR TRIM(LOWER(m.jenis_kategori)) = 'material stock')";
    } else {
...
/* QUERY UTAMA */
/* ==========================================================================
   QUERY UTAMA: ANTI-DUPLIKASI (MENGHILANGKAN KATEGORI STOK SILUMAN)
   ========================================================================== */
$query = mysqli_query($conn,"
    SELECT 
        ...
        COALESCE(ba.asal_barang_vendor, m.sumber_barang) AS sumber_barang,
        COALESCE(ba.keterangan, m.keterangan) AS keterangan
    FROM material_gudang m
    LEFT JOIN (
        SELECT 
            TRIM(LOWER(nama_barang)) AS key_nama,
            TRIM(LOWER(jenis_kategori)) AS key_kategori,
            MAX(asal_barang_vendor) AS asal_barang_vendor,
            MAX(keterangan) AS keterangan
        FROM database_ba
        WHERE nama_barang <> '' AND nama_barang IS NOT NULL
        GROUP BY TRIM(LOWER(nama_barang)), TRIM(LOWER(jenis_kategori))
    ) ba ON TRIM(LOWER(m.nama_material)) = ba.key_nama 
        AND TRIM(LOWER(m.jenis_kategori)) = ba.key_kategori -- << INI KUNCI UTAMANYA AGAR STOK TIDAK IKUT MUNCUL
    WHERE $whereClause
    ORDER BY
        CASE
            WHEN m.no_rak='A1' THEN 1
            WHEN m.no_rak='A2' THEN 2
            WHEN m.no_rak='A3' THEN 3
            WHEN m.no_rak='B1' THEN 4
            WHEN m.no_rak='B2' THEN 5
            WHEN m.no_rak='B3' THEN 6
            WHEN m.no_rak='B4' THEN 7
            WHEN m.no_rak='C1' THEN 8
            WHEN m.no_rak='C2' THEN 9
            WHEN m.no_rak='C3' THEN 10
            WHEN m.no_rak='D1' THEN 11
            WHEN m.no_rak='D2' THEN 12
            WHEN m.no_rak='D3' THEN 13
            WHEN m.no_rak='E1' THEN 14
            WHEN m.no_rak='E2' THEN 15
            WHEN m.no_rak='E3' THEN 16
            WHEN m.no_rak='F1' THEN 17
            WHEN m.no_rak='F2' THEN 18
            WHEN m.no_rak='G1' THEN 19
            WHEN m.no_rak='G2' THEN 20
            WHEN m.no_rak='G3' THEN 21
            WHEN m.no_rak='H1' THEN 22
            WHEN m.no_rak='H2' THEN 23
            WHEN m.no_rak='H3' THEN 24
            WHEN m.no_rak='I1' THEN 25
            WHEN m.no_rak='I2' THEN 26
            WHEN m.no_rak='J1' THEN 27
            WHEN m.no_rak='J2' THEN 28
            WHEN m.no_rak='K1' THEN 29
            WHEN m.no_rak='K2' THEN 30
            WHEN m.no_rak='K3' THEN 31
            WHEN m.no_rak='M1' THEN 32
            WHEN m.no_rak='M2' THEN 33
            WHEN m.no_rak='M3' THEN 34
            WHEN m.no_rak='PETI' THEN 35
            WHEN m.no_rak='RAK ISOLATOR' THEN 36
            ELSE 999
        END ASC,
        m.nama_material ASC
    LIMIT $offset,$limit
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .autocomplete-box {
            position: absolute; left: 0; right: 0; top: 100%; background: #ffffff; border: none; border-radius: 12px;
            margin-top: 6px; z-index: 99999 !important; max-height: 260px; overflow-y: auto; box-shadow: none; padding: 0;
        }
        .autocomplete-box.show-box {
        ...
        .kat-exbongkar { background: #f3e8ff; color: #6b21a8; border: 1px solid #e9d5ff; }
        ...
        .btn-action-item.btn-view:hover { color: var(--primary); background: rgba(2, 132, 199, 0.08); }
        ...
        .pagination .page-item.active .page-link { background: #0284c7 !important; color: #ffffff !important; border: none !important; box-shadow: 0 4px 15px rgba(2, 132, 199, 0.25); }
    </style>
</head>
<body>
...
<div class="sidebar">
    ...
    <div class="dropdown-container">
        <a href="index.php?kategori=stok">Stok</a>
        <a href="index.php?kategori=non stock">Non Stok</a>
        <a href="index.php?kategori=non po">Non PO</a>
        <a href="index.php?kategori=ex bongkaran">Ex Bongkaran</a>
        <a href="index.php?kategori=pre memory">Pre Memory</a>
        <a href="index.php?kategori=pemakaian">Pemakaian</a>
        <a href="index.php?kategori=peminjaman">Peminjaman</a>
    </div>
...
<div class="content">
    ...
    <div class="main-body-wrapper">
                    ...
                    <div class="col-md-10" style="position: relative;">
                        <div class="input-group input-cyber-group">
                            ...
                            <input 
                                ...
                                value="<?= htmlspecialchars($cari_clean, ENT_QUOTES, 'UTF-8'); ?>"
                            >
                        </div>
        ...
        <div class="mb-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
            <a href="tambah.php" class="btn btn-primary btn-sm fw-bold px-4 py-2" style="border-radius: 12px; background: var(--primary); border:none;"><i class="fa-solid fa-plus-circle me-1"></i> Registrasi Material Baru</a>
            <div>
                <?php if(!empty($kategori_db)) { ?>
                    <span class="badge bg-secondary p-2 me-2">Kategori Aktif: <?= strtoupper($kategori_db) ?> <a href="index.php" class="text-white text-decoration-none ms-1">&times;</a></span>
                <?php } ?>
            ...
            <table class="table-cyber">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            $kat = strtolower(trim($d['jenis_kategori'] ?? ''));
                            if($kat == 'stok' || $kat == 'material stock' || $kat == 'stock'){
                                echo '<span class="badge-kat kat-stock">Stok</span>';
                            } elseif($kat == 'non stock' || $kat == 'non-stock'){
                                echo '<span class="badge-kat kat-nonstock">Non Stock</span>';
                            } elseif($kat == 'non po' || $kat == 'non-po'){
                                echo '<span class="badge-kat kat-nonpo">Non PO</span>';
                            } elseif($kat == 'ex bongkaran' || $kat == 'ex-bongkaran'){
                                echo '<span class="badge-kat kat-exbongkar">Ex Bongkaran</span>';
                            } elseif($kat == 'pre memory' || $kat == 'pre-memory'){
                                echo '<span class="badge-kat kat-prememory">Pre Memory</span>';
                            } else {
                                echo '<span class="badge-kat kat-prememory">' . htmlspecialchars($d['jenis_kategori'] ?? '-', ENT_QUOTES, 'UTF-8') . '</span>';
                            }
                            ?>
                        </td>
                        ...
                        <td>
                            ...
                            if(strtoupper($d['kondisi'] ?? '') == 'BAIK'){
                                ...
                                echo "<span class='badge-status-other'><i class='fa-solid fa-triangle-exclamation me-1 small'></i> ".htmlspecialchars(strtoupper($d['kondisi'] ?: '-'), ENT_QUOTES, 'UTF-8')."</span>";
                            }
                            ?>
                        </td>
                        <td><i class="fa-solid fa-map-pin text-danger opacity-70 me-2 small"></i><?= htmlspecialchars($d['lokasi_penyimpanan'] ?: '-', ENT_QUOTES, 'UTF-8'); ?></td>
                        
                        <td>
                            <span class="small text-dark fw-semibold">
                                <i class="fa-solid fa-share-nodes text-primary opacity-50 me-1 small"></i>
                                <?= htmlspecialchars($d['sumber_barang'] ?: '-', ENT_QUOTES, 'UTF-8'); ?>
                            </span>
                        </td>
                        ...
                        <td class="text-center">
                            <div class="btn-action-group">
                                <a href="detail.php?id=<?= $d['id']; ?>" class="btn-action-item btn-view" title="Detail"><i class="fa-solid fa-expand"></i></a>
                                <a href="edit.php?id=<?= $d['id']; ?>" class="btn-action-item btn-edit" title="Ubah"><i class="fa-solid fa-user-pen"></i></a>
                                <a href="hapus.php?id=<?= $d['id']; ?>" class="btn-action-item btn-delete" title="Hapus" onclick="return confirm('Hapus permanen?')"><i class="fa-solid fa-trash-can"></i></a>
                            </div>
                        </td>
                    </tr>
                    <?php
                    ...
                    ?>
                    <tr>
                        <td colspan="11" class="text-center py-5" style="color: var(--text-muted) !important;">
                            <i class="fa-solid fa-satellite-dish d-block fs-1 mb-3 text-muted opacity-25"></i> Material tidak ditemukan.
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
...
<script>
    /* DROPDOWN SIDEBAR TOGGLE CONTROLLER */
