...
if(isset($_POST['update'])){
    ...
    if ($files_uploaded) {
        $files = $_FILES['foto'];
        // PERBAIKAN: Menghapus batasan ekstensi gambar saja agar semua format berkas (PDF, Excel, Doc, dsb) bisa masuk
...
<head>
    ...
    <style>
        /* WARNA DISESUAIKAN 100% SAMA DENGAN detail.php */
        :root { 
            --bg-body: #f4f7fc; 
            --bg-card: #ffffff; 
            --primary: #0284c7; 
        ...
        /* SAMAKAN PENUH: Penyelarasan kelas CSS Sidebar identik dengan detail.php */
        .sidebar {
        ...
        /* Icon representasi dokumen non-gambar */
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="jenis_kategori" class="form-select form-control-custom" required>
                                <?php $curr_kat = strtolower(trim($data['jenis_kategori'] ?? '')); ?>
                                <option value="Stok" <?= ($curr_kat == 'stok' || $curr_kat == 'material stock') ? 'selected' : ''; ?>>Stok</option>
                                <option value="Non Stock" <?= ($curr_kat == 'non stock' || $curr_kat == 'non-stock') ? 'selected' : ''; ?>>Non Stock</option>
                                        ...
                                        foreach($array_foto as $item_foto){
                                            ...
                                            if(!empty($item_foto) && file_exists("upload/".$item_foto)){
                                                ...
                                                if(in_array($ext, $img_exts)){
                                                    // Jika bertipe Gambar, tampilkan Thumbnail aslinya
                                                    echo '<img src="upload/'.$item_foto.'" class="img-edit-preview" alt="Barang" onclick="openPreview(\'upload/'.$item_foto.'\', true)">';
                                                } else {
                                                    // Jika bertipe Dokumen (PDF/Excel/dsb), berikan representasi Icon & link download/view
                                                    $icon = 'fa-file-lines';
...
<script>
    function openPreview(fileSrc, isImage) {
        if(isImage) {
            ...
            // Jika dokumen/non-gambar, otomatis buka tab baru untuk view / unduh berkas secara instan
    ...
    /* SAMAKAN PENUH: Fungsi javascript toggle menu yang sinkron penuh dengan detail.php */
