...
// Decode data foto dari database
$arr_np = json_decode($d['foto_nameplate'] ?? '[]', true);
if (!is_array($arr_np)) { $arr_np = []; }

$arr_mat = json_decode($d['foto_material'] ?? '[]', true);
if (!is_array($arr_mat)) { $arr_mat = []; }

// --- FITUR PROSES HAPUS LAMPIRAN SECARA INSTAN ---
if (isset($_GET['delete_file']) && isset($_GET['type'])) {
    $file_to_delete = $_GET['delete_file'];
    $type = $_GET['type'];
    $updated = false;

    if ($type === 'np' && ($key = array_search($file_to_delete, $arr_np)) !== false) {
        unset($arr_np[$key]);
        $arr_np = array_values($arr_np);
        $json_updated = mysqli_real_escape_string($conn, json_encode($arr_np));
        mysqli_query($conn, "UPDATE material_gudang SET foto_nameplate = '$json_updated' WHERE id = '$id'");
        $updated = true;
    } elseif ($type === 'mat' && ($key = array_search($file_to_delete, $arr_mat)) !== false) {
        unset($arr_mat[$key]);
        $arr_mat = array_values($arr_mat);
        $json_updated = mysqli_real_escape_string($conn, json_encode($arr_mat));
        mysqli_query($conn, "UPDATE material_gudang SET foto_material = '$json_updated' WHERE id = '$id'");
        $updated = true;
    }
    ...
    if ($updated) {
        ...
        echo "<script>alert('Foto berhasil dihapus!'); window.location.href='edit.php?id=$id';</script>";
        exit;
    }
}
...
if(isset($_POST['ubah'])){
    ...
    // --- PROSES UNGGAH BANYAK FOTO NAMEPLATE (APPEND / TAMBAH) ---
    ...
    // --- PROSES UNGGAH BANYAK FOTO MATERIAL (APPEND / TAMBAH) ---
    ...
    $update = "UPDATE material_gudang SET 
               nama_material = '$nama_material', 
               satuan = '$satuan', 
               jumlah = '$jumlah', 
               lokasi_penyimpanan = '$lokasi_penyimpanan',
               foto_nameplate = '$foto_nameplate_json',
               foto_material = '$foto_material_json',
               keterangan = '$keterangan'
               WHERE id = '$id'";
               
    if(mysqli_query($conn, $update)){
        echo "<script>alert('Data berhasil diperbarui!'); window.location='pre_memory.php';</script>";
        exit;
    } else {
...
<head>
    ...
    <title>I-CALM | Edit Pre Memory</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        /* IMAGE OVERLAY PREVIEW (TOMBOL X MELAYANG) */
        .gallery-item-wrapper { position: relative; display: inline-block; width: 100px; height: 75px; border-radius: 10px; overflow: hidden; border: 1px solid #cbd5e1; margin-right: 10px; margin-bottom: 10px; }
...
<div class="content">
    <nav class="navbar navbar-custom">
        <div class="container-fluid px-0">
            <span class="navbar-brand mb-0 h1 d-flex align-items-center">
                ...
                <span class="ms-2 fw-normal" style="font-size: 0.95rem; color: var(--text-muted);">/ Kategori: Pre Memory / Edit Data</span>
            </span>
        </div>
    </nav>
    ...
    <div class="main-body-wrapper">
        <div class="glass-card">
            <h4 class="fw-bold mb-4" style="color: #0f172a;"><i class="fa-solid fa-pen-to-square text-primary me-2"></i>Form Edit Data Pre Memory</h4>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="satuan" class="form-control" value="<?= htmlspecialchars($d['satuan']); ?>" required autocomplete="off">
                    </div>
                    
                    <div class="col-md-4">
                        <label class="form-label">Jumlah Volume</label>
                        <input type="number" name="jumlah" class="form-control" min="0" value="<?= $d['jumlah']; ?>" required>
                    </div>
                    
                    <div class="col-md-4">
                        <label class="form-label">Lokasi Penyimpanan</label>
                        <input type="text" name="lokasi_penyimpanan" class="form-control" value="<?= htmlspecialchars($d['lokasi_penyimpanan']); ?>" required autocomplete="off">
                    </div>

                    <!-- PREVIEW FOTO NAMEPLATE DENGAN TOMBOL X -->
                    <div class="col-md-6">
                        <label class="form-label">Foto Nameplate Saat Ini</label>
                        <div class="p-3 border bg-light mb-2" style="border-radius:10px; min-height:110px;">
                            <?php if(!empty($arr_np)): ?>
                                <div class="d-flex flex-wrap">
                                    <?php foreach($arr_np as $img): 
                                        $filepath = "upload/" . $img;
                                        if(file_exists($filepath)):
                                    ?>
                                        <div class="gallery-item-wrapper">
                                            <img src="<?= $filepath; ?>" class="preview-thumb" onclick="window.open(this.src, '_blank')" title="Klik untuk memperbesar">
                                            <a href="edit.php?id=<?= $id; ?>&delete_file=<?= urlencode($img); ?>&type=np" class="btn-delete-overlay" title="Hapus Foto" onclick="return confirm('Apakah Anda yakin ingin menghapus foto nameplate ini?');">
                            ...
                            <?php else: ?>
                                <span class="text-muted" style="font-size:0.8rem;"><i class="fa-solid fa-image-blur me-1"></i> Tidak ada foto nameplate.</span>
                            <?php endif; ?>
                        </div>
                        <label class="form-label mt-1">Tambah Foto Nameplate Baru</label>
                        <input type="file" name="foto_nameplate[]" class="form-control" multiple accept="image/*">
                    </div>

                    <!-- PREVIEW FOTO MATERIAL DENGAN TOMBOL X -->
                    <div class="col-md-6">
                        <label class="form-label">Foto Material Saat Ini</label>
                        <div class="p-3 border bg-light mb-2" style="border-radius:10px; min-height:110px;">
                            <?php if(!empty($arr_mat)): ?>
                                <div class="d-flex flex-wrap">
                                    <?php foreach($arr_mat as $img): 
                                        $filepath = "upload/" . $img;
                                        if(file_exists($filepath)):
                                    ?>
                                        <div class="gallery-item-wrapper">
                                            <img src="<?= $filepath; ?>" class="preview-thumb" onclick="window.open(this.src, '_blank')" title="Klik untuk memperbesar">
                                            <a href="edit.php?id=<?= $id; ?>&delete_file=<?= urlencode($img); ?>&type=mat" class="btn-delete-overlay" title="Hapus Foto" onclick="return confirm('Apakah Anda yakin ingin menghapus foto material ini?');">
                                                <i class="fa-solid fa-xmark"></i>
                                            </a>
                                        </div>
                                    <?php endif; endforeach; ?>
                            ...
                            <?php else: ?>
                                <span class="text-muted" style="font-size:0.8rem;"><i class="fa-solid fa-image-blur me-1"></i> Tidak ada foto material.</span>
                            <?php endif; ?>
                        </div>
                        <label class="form-label mt-1">Tambah Foto Material Baru</label>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="keterangan" class="form-control" rows="3" placeholder="Masukkan keterangan tambahan atau catatan berkas di sini..."><?= htmlspecialchars($d['keterangan'] ?? ''); ?></textarea>
                    </div>
                </div>
                
                <div class="mt-4 gap-2 d-flex">
                    <button type="submit" name="ubah" class="btn btn-primary px-4 fw-bold" style="border-radius:10px; background: #0284c7; border:none;"><i class="fa-solid fa-floppy-disk me-1"></i> Simpan Perubahan</button>
                    <a href="pre_memory.php" class="btn btn-light px-4 border" style="border-radius:10px;">Batal</a>
                </div>
            </form>
        </div>
    </div>
