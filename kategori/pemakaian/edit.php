...
// --- FITUR PROSES HAPUS LAMPIRAN (TANPA TAMBAH FILE) ---
if (isset($_GET['delete_file'])) {
    ...
    if (($key = array_search($file_to_delete, $arr_dokumentasi)) !== false) {
        ...
        // Hapus fisik file jika ada di folder upload
        ...
        mysqli_query($conn, "UPDATE material_gudang SET dokumentasi = '$json_updated' WHERE id = $id");
...
if (isset($_POST['update'])) {
    ...
    // --- PROSES UNGGAH MULTIPLE FOTO / BERKAS BARU ---
    ...
    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Data berhasil diperbarui!'); window.location.href='pemakaian.php';</script>";
        exit;
    } else {
...
// Mengambil daftar nomor rak unik untuk list dropdown
...
<head>
    ...
    <title>I-CALM | Edit Material Pemakaian</title>
...
<div class="content">
    <nav class="navbar navbar-custom">
        <div class="container-fluid px-0">
            <span class="navbar-brand mb-0 h1 d-flex align-items-center">
                ...
                <span class="ms-2 fw-normal" style="font-size: 0.95rem; color: var(--text-muted);">/ Kategori: Pemakaian / Edit Data</span>
            </span>
        </div>
    </nav>
    ...
    <div class="main-body-wrapper">
        <div class="glass-card">
            <h4 class="fw-bold mb-4" style="color: #0f172a;"><i class="fa-solid fa-pen-to-square text-warning me-2"></i>Edit Material Pemakaian</h4>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nama_material" class="form-control" value="<?= htmlspecialchars($data['nama_material']); ?>" required autocomplete="off">
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Satuan</label>
                        <input type="text" name="satuan" class="form-control" value="<?= htmlspecialchars($data['satuan']); ?>" required autocomplete="off">
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Jumlah Terpakai</label>
                        <input type="number" name="jumlah" class="form-control" value="<?= $data['jumlah']; ?>" required min="0">
                    </div>
                    
                    <div class="col-md-6">
                        <label class="form-label">Nomor Rak</label>
                        <?php $current_rak = $data['no_rak'] ?? $data['sumber_barang'] ?? ''; ?>
                        <select name="no_rak" id="no_rak" class="form-select" onchange="toggleRakBaru()">
                            ...
                            while($rak = mysqli_fetch_assoc($rak_query)): 
                                $selected = '';
                                if($rak['no_rak'] == $current_rak) { $selected = 'selected'; $rak_found = true; }
                            ?>
                                <option value="<?= htmlspecialchars($rak['no_rak']); ?>" <?= $selected; ?>><?= htmlspecialchars($rak['no_rak']); ?></option>
                            <?php endwhile; ?>
                            <option value="INPUT_BARU" <?= (!$rak_found && !empty($current_rak)) ? 'selected' : ''; ?>>* Input Rak Baru *</option>
                        </select>
                        
                        <div id="wrapper_rak_baru" class="mt-2" style="display: <?= (!$rak_found && !empty($current_rak)) ? 'block' : 'none'; ?>;">
                            <input type="text" name="no_rak_baru" class="form-control" value="<?= (!$rak_found) ? htmlspecialchars($current_rak) : ''; ?>" placeholder="Ketik nomor rak baru di sini..." autocomplete="off">
                        </div>
                    </div>
                        ...
                        <div class="p-3 border bg-light mb-2" style="border-radius:10px;">
                            ...
                            ?>
                                            <div class="gallery-item-wrapper">
                                                ...
                                                <a href="edit.php?id=<?= $id; ?>&delete_file=<?= urlencode($img); ?>" class="btn-delete-overlay" title="Hapus Gambar ini" onclick="return confirm('Apakah Anda yakin ingin menghapus gambar ini?');">
                            ...
                            <?php 
                                ...
                                // Tampilan khusus berkas Dokumen (PDF/Word)
                                if(!empty($docs)):
                            ...
                            ?>
                                            <div class="file-badge">
                                                ...
                                                <a href="edit.php?id=<?= $id; ?>&delete_file=<?= urlencode($doc); ?>" class="btn-delete-file" title="Hapus dokumen ini" onclick="return confirm('Apakah Anda yakin ingin menghapus dokumen ini?');">
                                                    <i class="fa-solid fa-xmark"></i>
                                                </a>
                                            </div>
                            <?php 
                ...
                <div class="mt-4 gap-2 d-flex">
                    <button type="submit" name="update" class="btn btn-warning px-4 fw-bold text-white" style="border-radius:10px; background-color: #f59e0b; border:none;"><i class="fa-solid fa-pen-to-square me-1"></i> Perbarui Data</button>
