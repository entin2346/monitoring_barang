...
if (isset($_POST['submit'])) {
    ...
    // Array untuk menampung nama-nama file yang diunggah
    $uploaded_files = [];

    // Proses unggah banyak berkas/foto
    ...
    // Mengonversi array nama file ke format JSON string untuk disimpan di database
    $lampiran_files = mysqli_real_escape_string($conn, json_encode($uploaded_files));

    $query = "INSERT INTO material_gudang (nama_material, jenis_kategori, satuan, jumlah, no_rak, kondisi, lokasi_penyimpanan, lampiran_files, tanggal) 
              VALUES ('$nama_material', '$jenis_kategori', '$satuan', $jumlah, '$no_rak', '$kondisi', '$lokasi_penyimpanan', '$lampiran_files', '$tanggal')";
...
<head>
    ...
    <title>I-CALM | Registrasi Non Stok Baru</title>
...
<div class="content">
    <nav class="navbar navbar-custom">
        <div class="container-fluid px-0">
            <span class="navbar-brand mb-0 h1 d-flex align-items-center" style="font-weight: 800; font-size: 1.3rem;">
                ...
                <span class="ms-2 fw-normal" style="font-size: 0.95rem; color: var(--text-muted);">/ Kategori: Non Stok / Registrasi Baru</span>
            </span>
        </div>
    </nav>
    ...
    <div class="main-body-wrapper">
        <div class="glass-form-card">
            <h4 class="fw-bold mb-4" style="color: #0f172a; letter-spacing: -0.02em;"><i class="fa-regular fa-square-plus text-primary me-2"></i>Form Entry Data Non Stok Baru</h4>
            
            <?php if(isset($error)): ?>
                <div class="alert alert-danger mb-4"><?= $error; ?></div>
            <?php endif; ?>

            <!-- Menambahkan atribut enctype="multipart/form-data" agar form mendukung upload file -->
            <form method="POST" enctype="multipart/form-data">
                <div class="row g-4">
                    ...
                    <div class="col-12">
                        <label class="form-label">Unggah Lampiran Dokumentasi / Berkas (Bisa Pilih Banyak Foto & File)</label>
                        <input type="file" name="files[]" class="form-control" multiple>
                        <div class="form-text text-muted small mt-1">Anda bisa memilih beberapa file sekaligus (Gambar, PDF, dll.) dengan menahan tombol Ctrl/Shift saat memilih file.</div>
                    </div>

                </div>

                <div class="mt-4 pt-2 d-flex gap-2">
                    <button type="submit" name="submit" class="btn-submit-ba">
                        <i class="fa-solid fa-floppy-disk me-1"></i> Simpan Data Non Stok
                    </button>
                    <a href="non_stok.php" class="btn-back-custom">Kembali Ke Halaman Utama</a>
                </div>
            </form>
        </div>
    </div>
